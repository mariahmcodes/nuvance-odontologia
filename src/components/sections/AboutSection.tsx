      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
        ...
        <div className="relative group w-full order-1">
          ...
          <div className="hidden lg:block relative">
            <Reveal>
              <div className="relative w-full aspect-[16/9] rounded-[2.5rem] overflow-hidden shadow-soft border border-white bg-petrol-soft/10">
                <img
                  src={aboutContent.image.src}
                  alt={aboutContent.image.alt}
                  width={aboutContent.image.width}
                  height={aboutContent.image.height}
                  fetchPriority={
                    aboutContent.image.priority ? "high" : undefined
                  }
                  decoding="async"
          ...
          <Reveal delay={0.24}>
            <ul className="space-y-4 mb-10 w-full" role="list">
              {aboutContent.features.map((item) => (
                ...
                >
                  <span
                    className="h-2 w-2 rounded-full bg-gold shrink-0 mt-2 transition-transform group-hover:scale-125"
                    aria-hidden="true"
                  />
