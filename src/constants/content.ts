export const brandAssets = {
  logo: {
    src: "/logo.webp",
    alt: "Nuvance Odontologia",
    width: 400,
    height: 400,
  },

  footerLogo: {
    src: "/logo2.png",
    alt: "Nuvance Odontologia",
    width: 500,
    height: 500,
  },

  texture: {
    src: "/gold-geometric-texture.webp",
    width: 1383,
    height: 700,
  },

  ogImage: {
    src: "/og-image.webp",
    width: 1200,
    height: 630,
    alt: "Nuvance Odontologia em Fortaleza",
  },
};

export const companyData = {
  whatsapp: "5585992945489",

  phoneRaw: "85992945489",

  whatsappBaseUrl:
    "https://api.whatsapp.com/send/?phone=5585992945489",

  whatsappLink:
    "https://api.whatsapp.com/send/?phone=5585992945489&text=Olá!%20Gostaria%20de%20agendar%20uma%20avaliação%20na%20Nuvance.",

  instagramUrl:
    "https://www.instagram.com/nuvanceodontologia/",

  address:
    "Av. Godofredo Maciel, 4000 — Fortaleza, CE",

  mapsUrl:
    "https://maps.google.com/?q=Av.+Godofredo+Maciel,+4000+Fortaleza+CE",

  schedule:
    "Seg–Sex • 08h–13h • 14h–18h",
};

export const heroContent = {
  eyebrow: "Odontologia premium",

  title: {
    main: "Implantes e estética com",
    highlight: "precisão e naturalidade.",
  },

  description:
    "25 anos transformando sorrisos em Fortaleza. Tecnologia digital, atendimento humanizado e resultados que duram a vida toda.",

  cta: {
    primary: "Agende sua avaliação",
    secondary: "Conheça os tratamentos",
  },
};

export const socialProofStats = [
  { value: "+2000", label: "Implantes realizados" },
  { value: "+6500", label: "Pacientes atendidos" },
  { value: "25", label: "Anos de experiência" },
  { value: "100%", label: "Odontologia digital" },
];

export const aboutContent = {
  eyebrow: "Sobre a Nuvance",

  title: {
    main: "Onde a precisão encontra a",
    highlight: "arte",
    complement: "do sorriso.",
  },

  description:
    "Localizada no bairro da Maraponga em Fortaleza, a Nuvance é um centro de excelência em reabilitação oral. Combinamos tecnologia de ponta com um olhar artístico para transformar vidas através da saúde bucal.",

  features: [
    "Experiência clínica de mais de 25 anos",
    "Fluxo 100% digital e tecnologia diagnóstica",
    "Ambiente acolhedor focado no conforto do paciente",
    "Excelência em casos de alta complexidade estética",
  ],

  badge: {
    value: "25+",
    label: "Anos cuidando de sorrisos",
  },

  cta: {
    label: "Conheça os especialistas",
    href: "#profissionais",
  },

  image: {
    src: "/images/about/team.webp",
    alt: "Equipe Nuvance Odontologia reunida na clínica",
    width: 1200,
    height: 675,
    priority: true,
  },
};

export const videoSectionContent = {
  eyebrow: "A clínica em movimento",

  title: "Um tour pelo nosso espaço",

  video: {
    src: "nKs2bcq_BFY",

    poster: "/images/video-thumb.webp",

    ariaLabel:
      "Vídeo institucional da clínica Nuvance",
  },
};

export const doctorsSectionContent = {
  eyebrow: "Nosso time",

  title: "Especialistas dedicados ao seu sorriso",

  description:
    "Profissionais que unem rigor técnico, sensibilidade estética e cuidado humano em cada consulta.",
};

export const doctors = [
  {
    name: "Dr. Thiago Carvalho",

    role: "Implantodontia",

    bio: "Especialista com foco em reabilitação oral e resultados naturais.",

    tags: ["Implantes", "Estética", "Reabilitação"],

    image: {
      src: "/images/doctors/dr-thiago.webp",

      alt: "Dr. Thiago Carvalho especialista em implantodontia",

      width: 960,

      height: 1408,
    },

    instagram: "https://www.instagram.com/drthiagocarvalho_/",
  },

  {
    name: "Dra. Juliana Maia",

    role: "Odontologia Estética",

    bio: "Atuação focada em estética dental e reabilitação oral.",

    tags: ["Estética", "Lentes de contato", "Facetas"],

    image: {
      src: "/images/doctors/dra-juliana.webp",

      alt: "Dra. Juliana Maia especialista em odontologia estética",

      width: 960,

      height: 1266,
    },

    instagram: "https://www.instagram.com/drajulianamaia/",
  },
];

export const treatmentsSectionContent = {
  eyebrow: "Tratamentos odontológicos",

  title: "Soluções pensadas pra cada etapa do seu sorriso",
};

export const treatments = [
  {
    icon: "smile",

    title: "Implantes Dentários",

    desc: "Reabilitação completa com técnica de carga imediata e precisão digital.",
  },

  {
    icon: "sparkles",

    title: "Facetas & Lentes",

    desc: "Transformação estética minimamente invasiva, com naturalidade absoluta.",
  },

  {
    icon: "award",

    title: "Reabilitação Oral",

    desc: "Devolvemos função, conforto e beleza ao seu sorriso por completo.",
  },

  {
    icon: "heart",

    title: "Odontologia Digital",

    desc: "Planejamento 3D, scanner intraoral e previsibilidade em cada etapa.",
  },
];

export const casesContent = {
  eyebrow: "Antes & depois",

  title: "Transformações que falam por si",

  description:
    "Cada caso é único. Veja como devolvemos confiança, função e beleza a centenas de pacientes.",

  testimonial: {
    quote:
      "Recuperei mais que meu sorriso — recuperei a confiança de sorrir de novo.",

    author: "Paciente Nuvance",
  },
};

export const casesItems = [
  {
    id: 1,

    type: "video",

    video: {
      src: "/videos/case-video-1.mp4",

      poster: "/videos/posters/case-1.webp",

      preload: "none",
    },

    title: "Caso #01",

    treatment: "Implantes + Facetas",
  },

  {
    id: 2,

    type: "image",

    image: {
      src: "/images/cases/case-image-1.webp",

      alt: "Resultado de lentes de contato dental",

      width: 960,

      height: 1200,
    },

    title: "Caso #02",

    treatment: "Lentes de Contato",
  },

  {
    id: 3,

    type: "video",

    video: {
      src: "/videos/case-video-2.mp4",

      poster: "/videos/posters/case-2.webp",

      preload: "none",
    },

    title: "Caso #03",

    treatment: "Protocolos sobre implantes",
  },

  {
    id: 4,

    type: "image",

    image: {
      src: "/images/cases/case-image-2.webp",

      alt: "Resultado de facetas em resina",

      width: 960,

      height: 1200,
    },

    title: "Caso #04",

    treatment: "Facetas em resina",
  },
] as const;

export const structureContent = {
  eyebrow: "Infraestrutura",

  title: "Um espaço pensado para você se sentir em casa.",

  description:
    "Ambientes amplos, design contemporâneo e equipamentos de última geração. Cada detalhe foi escolhido para transformar a experiência de ir ao dentista em um momento de cuidado e bem-estar.",

  features: [
    "Scanner intraoral e radiografia digital",
    "Salas de atendimento individuais e privativas",
    "Biossegurança em padrão hospitalar rigoroso",
  ],

  gallery: [
    {
      src: "/images/structure/clinic-1.webp",

      alt: "Consultório moderno da clínica Nuvance",

      width: 1024,

      height: 576,

      label: "Consultório 1",
    },

    {
      src: "/images/structure/clinic-2.webp",

      alt: "Ambiente clínico da Nuvance",

      width: 1024,

      height: 576,

      label: "Consultório 2",
    },

    {
      src: "/images/structure/clinic-3.webp",

      alt: "Sala de atendimento odontológico",

      width: 1024,

      height: 576,

      label: "Consultório 3",
    },

    {
      src: "/images/structure/clinic-4.webp",

      alt: "Corredor interno da clínica Nuvance",

      width: 1024,

      height: 576,

      label: "Corredor",
    },

    {
      src: "/images/structure/clinic-5.webp",

      alt: "Espaço administrativo da clínica",

      width: 1024,

      height: 576,

      label: "Gerência",
    },

    {
      src: "/images/structure/clinic-6.webp",

      alt: "Recepção da clínica Nuvance",

      width: 1024,

      height: 576,

      label: "Recepção",
    },

    {
      src: "/images/structure/clinic-7.webp",

      alt: "Fachada da clínica Nuvance em Fortaleza",

      width: 1024,

      height: 576,

      label: "Fachada Nuvance",
    },
  ],
};

export const ctaContent = {
  eyebrow: "Atendimento Exclusivo",

  title: "Sua jornada para um sorriso extraordinário.",

  highlight: "sorriso extraordinário.",

  description:
    "Estamos prontos para transformar sua experiência odontológica. Escolha como prefere falar conosco agora mesmo.",

  primary: "Agende pelo WhatsApp",

  secondary: "Ligue agora",
};

export const locationContent = {
  name: "Nuvance Odontologia",

  eyebrow: "Onde estamos",

  title: "Visite a Nuvance",

  highlight: "em Fortaleza",

  description:
    "Nossa clínica está localizada na Av. Godofredo Maciel, em uma região de fácil acesso, com ambiente exclusivo, seguro e estacionamento privativo para seu conforto.",

  addressLabel: "Endereço",

  address: {
    street: "Av. Godofredo Maciel, 4000 - Loja 02",

    district: "Mondubim",

    city: "Fortaleza",

    state: "CE",

    zip: "60711-502",

    country: "BR",
  },

  phone: "+55 85 99294-5489",

  mapQuery:
    "Nuvance Odontologia Av Godofredo Maciel 4000 Loja 02 Mondubim Fortaleza CE",

  mapEmbed:
    "Av.+Godofredo+Maciel,+4000+Loja+02,+Mondubim,+Fortaleza+CE,+60711-502",
};

export const testimonialsContent = {
  eyebrow: "Relato Real",

  title: "O melhor investimento da vida",

  video: {
    src: "/videos/testimonial.mp4",

    poster: "/videos/posters/testimonial.webp",

    preload: "metadata",

    ariaLabel:
      "Paciente relata que o tratamento na Nuvance foi o melhor investimento de sua vida",
  },
};